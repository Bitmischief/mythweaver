import { BillingInterval, BillingPlan, ProcessedStripeEvents, User } from '@prisma/client';
import { prisma } from '../../lib/providers/prisma';
import { AppError, HttpCode } from '../../lib/errors/AppError';

export class BillingDataProvider {
  async findProcessedStripeEvent(eventId: string): Promise<ProcessedStripeEvents | null> {
    return prisma.processedStripeEvents.findUnique({
      where: { eventId },
    });
  }

  async createProcessedStripeEvent(eventId: string, data: any): Promise<ProcessedStripeEvents> {
  ...
  async getLatestCampaignForUser(userId: number): Promise<{ name: string } | null> {

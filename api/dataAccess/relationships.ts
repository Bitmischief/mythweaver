...
export async function updateRelationship(id: number, data: any): Promise<any> {
  try {
    ...
    throw new Error('Could not update relationship');
  }
}
 ...
 * @returns {Promise<void>} A promise that resolves when the relationship is deleted.
 */
export async function deleteRelationship(id: number): Promise<void> {
  try {
    ...
    throw new Error('Could not delete relationship');
  }
}
